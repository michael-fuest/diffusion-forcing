...
class TimestepEmbedder(nn.Module):
    ...
    def timestep_embedding(t, dim, max_period=10000):
        """
        Create sinusoidal timestep embeddings for 2D t.
        :param t: a 2-D Tensor of shape [batch_size, num_frames]
        :param dim: the dimension of the output.
        :param max_period: controls the minimum frequency of the embeddings.
        :return: an (batch_size, num_frames, D) Tensor of positional embeddings.
        """
        half = dim // 2
        freqs = torch.exp(
            -math.log(max_period)
        ...
        # t: [batch_size, num_frames]
        # freqs: [128]
        # args: [batch_size, num_frames, 128]
        args = t[:, :, None].float() * freqs[None, None, :]  # [batch_size, num_frames, 128]
        embedding = torch.cat([torch.cos(args), torch.sin(args)], dim=-1)  # [batch_size, num_frames, 256]
        if dim % 2:
            embedding = torch.cat(
    ...
    def forward(self, t, use_fp16=False):
        ...
        t_emb = self.mlp(t_freq)  # [batch_size, num_frames, hidden_size]
        return t_emb
...
class Latte(nn.Module):
    ...
    def __init__(
        ...
        mlp_ratio=4.0,
        num_frames=16,
        class_dropout_prob=0.1,  # not used
        num_classes=-1,
        extras=1,
        use_checkpoint=False,
        attention_mode="math",
        ...
        self.vocab_embedding = nn.Embedding(vocab_size, hidden_size // 2)
        self.x_embedder = PatchEmbed(
            input_size, patch_size, hidden_size // 2, hidden_size, bias=True
        )
        ...
        self.temp_embed = nn.Parameter(
            torch.zeros(1, num_frames, hidden_size), requires_grad=False
        )
    ...
    def forward_with_cfg(
        ...
        use_fp16: bool = False,
        cfg_scale: Union[float, torch.FloatTensor, torch.DoubleTensor] = None,
    ):
        """
    ...
    def forward_without_cfg(
        ...
        return self.forward(
            x, t, y, text_embedding, use_fp16=use_fp16, cond_drop_prob=0.0
        )
    ...
    def forward(
        ...
        x: (N, F, H, W) tensor of video inputs
        ...
        # Prepare timestep embeddings for spatial and temporal processing
        timestep_spatial = repeat(t, "n f d -> (n f) d", n=bs)  # [N*F, hidden_size]
        timestep_temp = repeat(t, "n f d -> (n f) d", n=bs)  # [N*F, hidden_size]

        if self.extras == 2:
            y = self.y_embedder(y, self.training, cond_drop_prob=cond_drop_prob)  # [N, hidden_size]
            y_spatial = repeat(y, "n d -> (n f) d", f=self.num_frames)  # [N*F, hidden_size]
            y_temp = repeat(y, "n d -> (n f) d", f=self.num_frames)  # [N*F, hidden_size]
        elif self.extras == 78:
            raise NotImplementedError("Not implemented yet")
            # Handle text embeddings if needed

        # Pass through Transformer Blocks
        for i in range(0, len(self.blocks), 2):
            spatial_block, temp_block = self.blocks[i : i + 2]
            if self.extras == 2:
                c = timestep_spatial + y_spatial  # [N*F, hidden_size]
            elif self.extras == 78:
            ...
            x = spatial_block(x, c)  # [N*F, num_patches, hidden_size]
            x = rearrange(x, "(b f) p d -> (b p) f d", b=bs, p=x.shape[1])  # [N*P, F, hidden_size]

            # Add Temporal Embedding
            x = x + self.temp_embed  # [N*P, F, hidden_size]

            if self.extras == 2:
                c = timestep_temp + y_temp  # [N*P, hidden_size]
            elif self.extras == 78:
            ...
            x = temp_block(x, c)  # [N*P, F, hidden_size]
            x = rearrange(x, "(b t) p d -> (b t) p d", b=bs, t=x.shape[1])  # [N*F, P, hidden_size]
        ...
        # Final Layer
        x = self.final_layer(x, c)  # [N*F, P * patch_size**2 * vocab_size]
        x = self.unpatchify(x)  # [N*F, C, H, W]
        x = rearrange(x, "(b f) c h w -> b c f h w", b=bs)  # [N, C, F, H, W]
        return x  # [N, vocab_size, F, H, W]


    @property
    def param_num_embed_table(self):
        return sum(p.numel() for p in self.vocab_embedding.parameters())
    ...
    def forward_with_cfg(
        ...
        model_out = self.forward(
            combined, t, y=y, use_fp16=use_fp16, text_embedding=text_embedding
        )
...
if __name__ == "__main__":
    ...
    img = torch.randint(0, vocab_size, (3, 16, 32, 32)).to(device)
    t = torch.tensor([1, 2, 3]).to(device)
    y = torch.tensor([1, 2, 3]).to(device)
    network = Latte_B_2(vocab_size=vocab_size).to(device)
